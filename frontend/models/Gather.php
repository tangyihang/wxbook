...
use frontend\modelsCLS\HttpCLS;
use frontend\modelsCLS\ChineseCLS;
use frontend\modelsFUN\CharsetFUN;
use frontend\modelsDB\BooksourceDB;
use frontend\modelsDB\CollectRuleDB;
use frontend\modelsDB\CollectFieldDB;
use frontend\modelsDB\InfobookDB;
use frontend\modelsDB\InfochapterDB;
use frontend\modelsDB\TicketmutexDB;
use yii\helpers\VarDumper;
use frontend\modelsDB\MongoChapterDB;
use frontend\modelsDB\CollectSourceDB;
/**
 * 采集类
 * @author yihang
 *
		...
		foreach ( $urlregions as $urlregion ) { // 遍历每个url内容区块
			...
			if (!empty($contents)) {
				if (($row = BooksourceDB::getBookSourceByDB($contents['bookinfourl']))) {//判断该来源书籍是否已经存在
					...
					if (InfochapterDB::filter_mark($contents['bcnewtitle']) != InfochapterDB::filter_mark($row->bcnewtitle)) {
						//更新来源信息
...
// 						var_dump($infocontents);die;
	...
	function fetch_book_chapters($booksource) {
		...
		//声明mongo链接
		$contentcoll = Yii::$app->mongodb->getCollection('chapter_content');
		$sourcecoll = Yii::$app->mongodb->getCollection('chapter_source');
		//查询现有章节信息
		$chapters = InfochapterDB::getBookChapters($booksource->bookid);
		//查询书籍中该源的全部连接信息
		...
		if (!empty($urlregions)) {
			foreach ( $urlregions as $urlregion ) { // 遍历每个url内容区块
				...
				if (!empty($contents)) {
					//章节不存在时保存章节
					if (!isset($chapters[InfochapterDB::filter_mark($contents['chaptertitle'])])) {
						//获取章节信息和内容
						$chapterHtml = $this->onepage ( $contents['chapterurl'] );
						$chapterContents = $this->fetch_fields($this->chapterFields, $chapterHtml, $booksource->listurl);
						if (!empty($chapterContents)) {
							//保存章节信息
							$chapter = new InfochapterDB();
							$chapter->chaptertitle = $contents['chaptertitle'];
							$chapter->sourceid = $this->sourceid;
							$chapter->bookid = $booksource->bookid;
							$chapter->updatetime = time();
							if ($chapter->save()){
								//设置最新章节信息
								$bcnewchapter = $chapter;
								//保存章节内容
								$wordcount = globals::getStringWordCount($chapterContents['content']);
								$contentcoll->insert([
							...
							if ($this->chapterRule->errorcontent != $this->fetch_detail ( $this->chapterRule->errorregular, $chapterHtml )) {
								globals::setErrorLog(array('bookid' => $booksource->bookid, 'fields' => $this->chapterFields, 'html' => $chapterHtml, 'url' => $contents['chapterurl']));
								return false;die;
							} else {
								globals::setErrorLog(array('bookid' => $booksource->bookid, 'fields' => $this->chapterFields, 'html' => $chapterHtml, 'url' => $contents['chapterurl']), '404chapter');
							}
						}
					} else {
						...
						if (!in_array($contents['chapterurl'], $sources)) {
							$sourcecoll->insert([
									'chapterid' => $chapters[InfochapterDB::filter_mark($contents['chaptertitle'])]->chapterid,
									'bookid' => $booksource->bookid,
									'sourceid' => $this->sourceid,
									'csurl' => $contents['chapterurl'],
									'updatetime' => time(),
				...
				unset($contents, $chapterHtml, $chapterContents, $chapter, $wordcount);
			}
		} else {
			return false;
		}
		//如果有新增章节，更新书籍最新章节信息
		if (!empty($bcnewchapter)) {
			$book = InfobookDB::getBookById($booksource->bookid);
			if (!empty($book)) {
				$book->bcnew = $bcnewchapter->chapterid;
				$book->bcnewtitle = $bcnewchapter->chaptertitle;
				$book->updatetime = time();
				$book->save();
			}
		}
		var_dump('success_'.$booksource->bookid.'_'.$this->sourceid);
		unset($urlregions, $urlregion, $html, $contentcoll, $sourcecoll, $chapters, $sources, $bcnewchapter);
		return true;
	}
