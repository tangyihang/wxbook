 ...
 * 二手房聚合房源sphinx
 */
class BooksourceSphinx extends SphinxClient {
    ...
    public function __construct($server = '') {
        parent::__construct();
        $config = !empty($server) ? $server : \Yii::$app->params['sphinxServer']['booksource'];
    ...
    public function setBookid($bookid){
    	$this->SetFilter("bookid", array($bookid));
    }
     ...
     * @param int $sourceid
     */
    public function setSounceid($sourceid){
    	$this->SetFilter("sourceid", array($sourceid));
    }
    
    /**
     * 根据书籍源id数组搜索
     */
    public function setBsids($bsids){
    	$this->SetFilter("bsid", $bsids);
    }
